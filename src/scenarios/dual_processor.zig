//! Dual-Processor Stress Scenarios (30-32)
//!
//! Send buy/sell pairs across two symbols (IBM and NVDA) to
//! exercise both processors in the matching engine.
...
// ============================================================
// Dual-Processor Stress Test
// ============================================================

pub fn runDualProcessorStress(client: *EngineClient, stderr: std.fs.File, trades: u64) !void {
    const orders = trades * 2;
    const trades_per_proc = trades / 2;

    // Header
    if (trades >= 10_000_000) {
        try printUltimateHeader(stderr, trades, orders, trades_per_proc);
    } else {
        try helpers.print(stderr, "=== Dual-Processor Stress: {d} Trades ===\n\n", .{trades});
    }

    // Target info
    try printTarget(stderr, trades, orders, trades_per_proc);
    ...
    const batch_size = config.DUAL_PROC_BATCH_SIZE;
    ...
    if (trades >= 10_000_000 and running_stats.trades >= pairs_sent) {
        try printUltimateAchievement(stderr);
    }
...
fn printTarget(stderr: std.fs.File, trades: u64, orders: u64, trades_per_proc: u64) !void {
    if (trades >= 1_000_000) {
    ...
    try helpers.print(stderr, "  Processor 0 (A-M): IBM  - {d} trades\n", .{trades_per_proc});
    try helpers.print(stderr, "  Processor 1 (N-Z): NVDA - {d} trades\n", .{trades_per_proc});
}

fn printUltimateHeader(stderr: std.fs.File, trades: u64, orders: u64, trades_per_proc: u64) !void {
    try stderr.writeAll("\n");
    try stderr.writeAll("╔══════════════════════════════════════════════════════════╗\n");
    try stderr.writeAll("║  ★★★ DUAL-PROCESSOR STRESS TEST ★★★                      ║\n");
    try helpers.print(stderr, "║  {d}M TRADES ({d}M ORDERS)                              ║\n", .{ trades / 1_000_000, orders / 1_000_000 });
    try helpers.print(stderr, "║  Processor 0 (A-M): IBM  - {d}M trades                   ║\n", .{trades_per_proc / 1_000_000});
    try helpers.print(stderr, "║  Processor 1 (N-Z): NVDA - {d}M trades                   ║\n", .{trades_per_proc / 1_000_000});
    try stderr.writeAll("╚══════════════════════════════════════════════════════════╝\n");
    try stderr.writeAll("\n");
}

fn printUltimateAchievement(stderr: std.fs.File) !void {
    try stderr.writeAll("\n╔══════════════════════════════════════════════════════════╗\n");
