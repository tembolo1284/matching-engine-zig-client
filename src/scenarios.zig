...
fn runStressTest(client: *EngineClient, stderr: anytype, count: u32) !void {
    ...
    const batch_size: u32 = if (count >= 1_000_000) 50_000 
                           else if (count >= 100_000) 10_000 
                           else if (count >= 10_000) 1_000 
                           else count;
...
fn runBurstStress(client: *EngineClient, stderr: anytype, count: u32) !void {
    ...
    if (count > send_errors) {
        ...
        try stderr.print("\n*** This is CLIENT-SIDE throughput only! ***\n", .{});
        try stderr.print("*** Check server for actual processing rate ***\n", .{});
    }
...
// ============================================================
// Response Handling (shared with interactive mode)
// ============================================================
